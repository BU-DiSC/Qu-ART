            ...
            void insert(ART* tree, ArtNode* node, ArtNode** nodeRef, uint8_t key[], unsigned depth,
                ...
                if (isLeaf(node)) {
                    ...
                    *nodeRef = newNode;
                    ...
                    node->insertNode4(tree, newNode, nodeRef, existingKey[depth + newPrefixLength], node, temp_fp_path, temp_fp_path_length);
                    insertNode4(tree, newNode, nodeRef, key[depth + newPrefixLength], makeLeaf(value), temp_fp_path, temp_fp_path_length);
                    return;
                }
                    ...
                    if (mismatchPos != node->prefixLength) {
                        ...
                        *nodeRef = newNode;
                        ...
                        if (node->prefixLength < maxPrefixLength) {
                            ...
                            insertNode4(tree, newNode, nodeRef, node->prefix[mismatchPos], node, temp_fp_path, temp_fp_path_length);
                            ...
                            insertNode4(tree, newNode, nodeRef, minKey[depth + mismatchPos],
                                        node, temp_fp_path, temp_fp_path_length); 
                            memmove(node->prefix, minKey + depth + mismatchPos + 1,
                                    min(node->prefixLength, maxPrefixLength));
                        }
                        insertNode4(tree, newNode, nodeRef, key[depth + mismatchPos],
                                    makeLeaf(value), temp_fp_path, temp_fp_path_length); // adds to the fp path
                        return;
                    }
                ...
                if (*child) {  
                    insert(tree, *child, child, key, depth + 1, value, maxKeyLength, temp_fp_path, temp_fp_path_length);
                    return;
                }

                // Insert leaf into inner node, last step of the function
                ArtNode* newNode = makeLeaf(value);
                switch (node->type) {
                    case NodeType4:
                        insertNode4(tree, static_cast<Node4*>(node), nodeRef, key[depth],
                                    newNode, temp_fp_path, temp_fp_path_length);
                        break;
                    case NodeType16:
                        insertNode16(tree, static_cast<Node16*>(node), nodeRef, key[depth],
                                    newNode, temp_fp_path, temp_fp_path_length);
                        break;
                    case NodeType48:
                        insertNode48(tree, static_cast<Node48*>(node), nodeRef, key[depth],
                                    newNode, temp_fp_path, temp_fp_path_length);
                        break;
                    case NodeType256:
                        insertNode256(tree, static_cast<Node256*>(node), nodeRef, key[depth],
                                    newNode, temp_fp_path, temp_fp_path_length);
                        break;
                }
            }
