...
class QuART_stail_reset : public ART {
   private:
    int reset_counter;

   public:
    QuART_stail_reset() : ART(), reset_counter(300) {}
    ...
    void insert(uint8_t key[], uintptr_t value) {
        ...
        if (root == nullptr) {
            QuART_stail_reset::insert_recursive_change_fp(
        ...
        for (size_t i = 0; i < maxPrefixLength - 1; i++) {
            ...
            // only update the current fp information if it changes.
                ...
                QuART_stail_reset::insert_recursive_preserve_fp(
                    this->root, &this->root, key, 0, value, maxPrefixLength);
                return;
            }
