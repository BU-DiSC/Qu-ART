...
void insertNode48(ART* tree, Node48* node, ArtNode** nodeRef, uint8_t keyByte,
                  ArtNode* child) {
    // Insert leaf into inner node
    if (node->count < 48) {
        // Insert element
...
void eraseNode4(ART* tree, Node4* node, ArtNode** nodeRef,
                ArtNode** leafPlace) {
...
void eraseNode16(ART* tree, Node16* node, ArtNode** nodeRef,
                 ArtNode** leafPlace) {
    // Delete leaf from inner node
    unsigned pos = leafPlace - node->child;
    memmove(node->key + pos, node->key + pos + 1, node->count - pos - 1);
...
void eraseNode256(ART* tree, Node256* node, ArtNode** nodeRef,
                  uint8_t keyByte) {
    // Delete leaf from inner node
